...
export default function (Vue, options) {
    ...
    function handleDragEnd (e) {
        ...
        const eq = el.getAttribute('drag_key');
        isImpact(e, el.drag_target) 
            ?  $dragging.$emit('dragend', { group, eq, target: el, item: el.drag_mess })
            : null;
    }

    function isImpact(e, target) {
        const dragEndPoints = {
            top: e.clientY,
            right: e.clientX,
            bottom: e.clientY,
        ...
        const targetOpts = target.getBoundingClientRect();
        if(
            dragEndPoints.bottom < targetOpts.top
            || dragEndPoints.left > targetOpts.right
            || dragEndPoints.top > targetOpts.bottom
            || dragEndPoints.right < targetOpts.left) {
                console.log('没碰了')
                return false;
        }else {
            console.log('碰了') 
    ...
    function getDragTarget(binding, vnode, tar) {
        if(!binding.value[tar]) return false;
        const context = vnode.context;
        var target = context;
        while(target.$parent) {
            if(target.$refs[binding.value[tar]]) {
                return target.$refs[binding.value[tar]];
            }else {
    ...
    function addDragItem (el, binding, vnode) {
        ...
        const target = getDragTarget(binding, vnode, 'target');

        const DDD = dragData.new(binding.value.group)

        const drag_key = binding.value.key
